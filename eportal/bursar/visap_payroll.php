            ...
            <table class="table">
              ...
              <tbody class="text-center">
                ...
                foreach ($get_all_payrolls as $payrolls) {
                  ...
                  if ($staff_data->staffGender =="Male") {
                    $herTittle = "Mr";
                  }elseif ($staff_data->staffGender =="Female") {
                   $herTittle = "Mrs";
                  }else{
                     $herTittle = "Unknown";
                  }
                  ?>
                  <tr>
                    <td><?php echo ucwords($herTittle." ".$staff_data->firstName." ".$staff_data->lastName);?></td>
                    <td><?php echo ucfirst($staff_data->staffRole);?></td>
                    <td><?php echo ucfirst($staff_data->staffEducation);?></td>
                    <td><?php echo date("j F Y",strtotime($staff_data->appliedDate)) ?></td>
              ...
              <div class="dropdown-menu" aria-labelledby="dropdownMenuOffset">
                 <a class="dropdown-item text-info view_bank_details" data-id="<?php echo $payrolls->staff_id;?>" href="javascript:void(0);"> Bank Details</a>
...
<!-- BANK MODAL -->
      ...
      <span aria-hidden="true">&times;</span>
          ...
          <div class="card-opening text-center"><h2>Bank Information</h2></div>
            <div class="card-desc">
                <!-- if transfer -->
                <h4> Bank: <b id="myBankName">FIRST BANK PLC</b></h4>
                <h4>Account Name: <b id="myBankAccountName">AGBERAYI IDOWU SAM</b></h4>
                <h4> Account No: <b class="text-danger" id="myBankAccountNumber">3107991990</b></h4>
                <h4>Account Type: <b> <span class="badge badge-warning text-white badge-lg" id="myBankAccountType"> Saving Account</span></b></h4>
            </div>
            <!-- Ends -->
...
<script>
 ...
 $(".__loadingBtn__").html('Submit').attr("disabled",false);
  $("#responseText").html(data);
   ...
   //when bank details if viewed
   const view_bank_details =$(".view_bank_details");
   view_bank_details.on("click", function(){
   //#viewbankInfoModalForm
   let staff_keyId = $(this).data("id"),action_p ='view_bank_details';
   //alert(action_p);
   $.post("../actions/update_actions",{action:action_p,staff_id:staff_keyId},function(data){
    $("#myBankName").html(data.bank_name);
    $("#myBankAccountName").html(data.account_name);
    $("#myBankAccountNumber").html(data.account_number);
    $("#myBankAccountType").html(data.account_type);
