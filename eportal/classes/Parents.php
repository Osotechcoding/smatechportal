...
class Parents
{
	...
	public function registerParent(array $data)
	{
		...
		if ($this->config->isEmptyStr($fname) || $this->config->isEmptyStr($lname) || $this->config->isEmptyStr($phone) || $this->config->isEmptyStr($email) || $this->config->isEmptyStr($occupation) || $this->config->isEmptyStr($address) || $this->config->isEmptyStr($gender) || $this->config->isEmptyStr($pta_status)) {
			...
			try {
				...
				$query = "INSERT INTO `{$this->table}` (fname,lname,phone,email,`address`,occupation,gender,pta_post,created_at) VALUES (?,?,?,?,?,?,?,?,?);";
				$this->stmt = $this->dbh->prepare($query);
				if($this->stmt->execute([$fname,$lname,$phone,$email,$address,$occupation,$gender,$pta_status,$datetime])){
					$this->dbh->commit();
